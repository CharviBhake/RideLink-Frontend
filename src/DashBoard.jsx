...
export default function CarpoolDashboard() {
  ...
  const getStats = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await fetch("http://localhost:8080/user/total_trips", {
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type":"application/json",
        },
...
useEffect(()=>{
  const getCo2=async()=>{
    try{
      const token=localStorage.getItem("token");
      const response=await fetch("http://localhost:8080/user/co2",{
        headers:{
          Authorization: `Bearer ${token}`,
...
useEffect(()=>{
  const getSavings= async()=>{
    try{
      const token=localStorage.getItem("token");
      const response=await fetch("http://localhost:8080/user/savings",{
        headers:{
          Authorization: `Bearer ${token}`,
          "Content-Type":"application/json",
        },
