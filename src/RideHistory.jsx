...
const [tempComment, setTempComment] = useState("");
  ...
  const fetchRideHistory = async () => {
    ...
    try {
      ...
      if (activeTab === 'driver') {
        ...
        const response = await fetch(`${import.meta.env.VITE_API_URL}/ride/history/driver`, {
          headers: {
        ...
        const response = await fetch(`${import.meta.env.VITE_API_URL}/ride/history/passenger`, {
          headers: {
             Authorization: `Bearer ${token}`,
            "Content-Type":"application/json",
          }
  ...
  useEffect(()=>{
    const getSavings= async()=>{
      try{
        const token=localStorage.getItem("token");
        const response=await fetch(`${import.meta.env.VITE_API_URL}/ride/savings`,{
          headers:{
            Authorization: `Bearer ${token}`,
            "Content-Type":"application/json",
          },
  ...
  useEffect(()=>{
    const fetchReviews=async()=>{
      try{
        const token=localStorage.getItem("token");
        const res=await fetch(`${import.meta.env.VITE_API_URL}/review/my`,{
          headers:{
            Authorization: `Bearer ${token}`,
            "Content-Type":"application/json",
          },
